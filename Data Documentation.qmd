...
[@bureauoflaborstatistics67DetailedNature2023] Dataset R67 Detailed nature by number of days by case type
...
R10: Detailed occupation by selected parts of the body affected [@bureauoflaborstatisticsR10DetailedOccupation2023].
...
-   **Industries** are classified into goods-producing, like construction, and service-producing, like financial activities (including real estate and insurance), as laid out in the NCAIS codebook.

Detailed event or exposure by major industry division (Rate)Provides annualized incidence rates per 10,000 full-time workers for DART, DAFW, and DJTR cases sorted by event/exposure and industry sector. Since there are more service providing jobs in the dataset, the incidence rate must be used to obtain accurate proportion differences. Some data values are empty and must be cleaned. Dataset can be used to see the likelihood of injury.

R65: Detailed industry by number of days by case type*\

Reports the number of nonfatal injuries and illnesses involving days away from work (DAFW), restricted activity or job transfer (DJTR), or both (DART), sorted by industry and days missed. Dataset can be used to see the severity of injury through (DAFW).
